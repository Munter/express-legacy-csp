...
expect.addAssertion(
  ...
  (expect, subject, value) => {
    ...
    expect(
      normalizeParsedCsp(downgradeCsp2ToCsp1(normalizeParsedCsp(subject))),
      'to satisfy',
...
describe('downgradeCsp2ToCsp1', () => {
  ...
  it('should keep unsafe-inline when and drop nonces and hashes', () => {
    expect(
      "script-src 'sha256-XeYlw2NVzOfB1UCIJqCyGr+0n7bA4fFslFpvKu84IAw=' 'unsafe-inline' 'nonce-foo'",
      'to come out as',
      "script-src 'unsafe-inline'"
    );
  });
  ...
  it('should add unsafe-inline when nonces and hashes are dropped', () => {
    expect(
      "script-src 'sha256-XeYlw2NVzOfB1UCIJqCyGr+0n7bA4fFslFpvKu84IAw=' 'nonce-foo'",
      'to come out as',
      "script-src 'unsafe-inline'"
    );
  });
  ...
  it('should strip the path from a source expression with a port', () => {
    expect(
      'script-src somewhere.com:8888/with/a/path',
      'to come out as',
      'script-src somewhere.com:8888'
    );
  });
