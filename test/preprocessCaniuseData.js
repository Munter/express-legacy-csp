...
expect.addAssertion(
    '<object> [not] to support <string> <number+>',
    (expect, subject, caniuseId, major, minor, patch) =>
        expect(
            preprocessCaniuseData(subject)(caniuseId, major, minor, patch),
            '[not] to be true'
        )
);

expect.addAssertion(
    '<object> [not] to be undecided about <string> <number+>',
    (expect, subject, caniuseId, major, minor, patch) => {
...
describe('preprocessCaniuseData', function() {
    ...
    it('should let exact major.minor versions through', function() {
        expect({ ie: { '11.1': 'y' } }, 'to support', 'ie', 11, 1).and(
            'not to support',
            'ie',
            11,
            0
    ...
    it('should let exact major.minor.patch versions through', function() {
        expect({ ie: { '11.1.2': 'y' } }, 'to support', 'ie', 11, 1, 2).and(
            'not to support',
            'ie',
            11,
            1
