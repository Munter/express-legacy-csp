...
expect.addAssertion('<object|string> to come out as <object|string>', (expect, subject, value) => {
    ...
    expect(normalizeParsedCsp(downgradeCsp3ToCsp2(normalizeParsedCsp(subject))), 'to satisfy', normalizeParsedCsp(value));
});
...
describe('downgradeCsp3ToCsp2', () => {
    ...
    it("should leave 'unsafe-inline' when removing 'unsafe-hashed-attributes'", () => {
        expect("script-src 'unsafe-inline' 'unsafe-hashed-attributes'", 'to come out as', "script-src 'unsafe-inline'");
    });

    it("should leave 'nonce-...'", () => {
        expect("script-src 'nonce-foo'", 'to come out as', "script-src 'nonce-foo'");
    });
