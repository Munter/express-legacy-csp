...
const checkSupportedByLevelAndBrowserAndVersion = {
    1: preprocessCaniuseData(require('caniuse-db/features-json/contentsecuritypolicy.json').stats),
    2: preprocessCaniuseData(require('caniuse-db/features-json/contentsecuritypolicy2.json').stats)
};

try {
    checkSupportedByLevelAndBrowserAndVersion[3] = preprocessCaniuseData(require('caniuse-db/features-json/contentsecuritypolicy3.json').stats);
} catch (e) {}

const maxCspLevel = Math.max(...Object.keys(checkSupportedByLevelAndBrowserAndVersion).map(levelStr => parseInt(levelStr, 10)));
...
const lookupTargetCspLevelInCanIUseDb = memoizeSync((family, major, minor) => {
    ...
    if (caniuseId) {
        ...
        for (let cspLevel = maxCspLevel ; cspLevel > 0 ; cspLevel -= 1) {
            const isSupported = checkSupportedByLevelAndBrowserAndVersion[cspLevel](caniuseId, major, minor);
            if (isSupported) {
                return cspLevel;
            ...
            ['Content-Security-Policy', 'Content-Security-Policy-Report-Only'].forEach(headerName => {
                ...
                if (existingCspHeaderValues) {
                    ...
                    if (agent.family === 'IE' && agent.satisfies('11 || 10')) {
                        ...
                        targetCspLevel = 2;
                    } else if (agent.family === 'Mobile Safari') {
                        // We need to special case here until #4 is fixed
                        // caniuse-db uses version ranges almost exclusively for ios_saf
                        if (agent.satisfies('>= 9.3')) {
                            targetCspLevel = 2;
                        } else if (agent.satisfies('>= 6.1')) {
                            targetCspLevel = 1;
                        } else {
                            targetCspLevel = 0;
                        }
