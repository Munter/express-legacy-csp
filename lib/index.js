...
const lookupTargetCspLevelInCanIUseDb = memoizeSync((family, major, minor) => {
    ...
    if (caniuseId) {
        ...
        for (let cspLevel = maxCspLevel; cspLevel > 0; cspLevel -= 1) {
            const isSupported = checkSupportedByLevelAndBrowserAndVersion[
                cspLevel
            ](caniuseId, major, minor);
            if (isSupported) {
                return cspLevel;
        ...
        res.writeHead = (...args) => {
            [
                ...
                if (existingCspHeaderValues) {
                    ...
                    let reportOnlySuffix = /-Report-Only$/.test(headerName)
                        ? '-Report-Only'
                        : '';
                    let targetCspLevel;
                    let newHeaderName;
                    ...
                    if (targetCspLevel === 0) {
                        ...
                        res.removeHeader(headerName);
                        existingCspHeaderValues.forEach(
                            existingCspHeaderValue => {
                                res.append(
                                    newHeaderName || headerName,
                                    downgradeAndSerializeCsp(
                                        existingCspHeaderValue,
                                        targetCspLevel
                                    )
                                );
