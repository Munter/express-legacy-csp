...
module.exports = function preprocessCaniuseData(stats) {
  ...
  Object.keys(stats).forEach(caniuseId => {
    ...
    Object.keys(stats[caniuseId]).forEach(version => {
      ...
      if (version === 'all') {
        (checkSupportedRangeByCaniuseId[caniuseId] =
          checkSupportedRangeByCaniuseId[caniuseId] || []).push(
          () => isSupported
        );
        ...
        const matchRange = version.match(
          /^(\d+(?:\.\d+){0,2})-(\d+(?:\.\d+){0,2})$/
        );
        if (matchRange) {
    ...
    if (highestVersionSeen) {
      (checkSupportedRangeByCaniuseId[caniuseId] =
        ...
        (major, minor, patch) => {
          if (
            semver.gt(buildVersion(major, minor, patch), highestVersionSeen)
          ) {
    ...
    if (lowestVersionSeen) {
      (checkSupportedRangeByCaniuseId[caniuseId] =
        ...
        (major, minor, patch) => {
          if (
            semver.lt(buildVersion(major, minor, patch), highestVersionSeen)
          ) {
            return false;
          }
        }
  ...
  return function checkSupported(caniuseId, major, minor, patch) {
    ...
    if (isSupportedByVersion) {
      if (typeof patch !== 'undefined') {
        const cspIsSupportedInPatchVersion =
          isSupportedByCaniuseIdAndVersion[caniuseId][
            `${major}.${minor}.${patch}`
          ];
      ...
      const cspIsSupportedInMinorVersion =
        isSupportedByCaniuseIdAndVersion[caniuseId][`${major}.${minor}`];
      if (typeof cspIsSupportedInMinorVersion === 'boolean') {
        return cspIsSupportedInMinorVersion;
      }

      const cspIsSupportedInMajorVersion =
        isSupportedByCaniuseIdAndVersion[caniuseId][major];
    ...
    if (ranges) {
      for (var i = 0; i < ranges.length; i += 1) {
        const isRangeSupported = ranges[i](major, minor, patch);
